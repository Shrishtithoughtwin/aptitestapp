...
const {t}=useTranslation();
  ...
  return (
    <div className="container mx-auto p-6 text-center">
      ...
      {t("profitloss.description")}
      </p>
      <h2 className="text-3xl  mb-4">
      {t("profitloss.testHeading")}
      </h2>    
   ...
   {[1, 2].map((testId) => (
        ...
        >
          <div className="flex-1 text-left ml-2">
            <h2 className="text-lg mb-1">  {t("profitloss.onlineTest")} - {testId} ({t("profitloss.topic")})</h2>
            <p className="text-gray-500 mb-1 text-base">
            {t("profitloss.onlineTestDescription")}
            </p>
          </div>
          <button
          ...
          >
           {t("profitloss.takeTestButton")}
          </button>
        </div>
      ))}
      <div className="bg-blue-400 h-1 my-6"></div>
      <h1 className="text-2xl  font-bold mb-3"> {t("profitloss.mockTestTitle")}</h1>
      <p className="mb-3 text-gray-500  text-justify">
      {t("profitloss.mockTestDescription")}
      </p>
    </div>
  );
};
